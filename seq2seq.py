...
class Encoder(nn.Module):
    ...
    def __init__(self, n_lands=10, n_acts=3, hidden_size=128, emb_dim=32,
        ...
        if not orientation_aware:
            self.action_embedding = nn.Embedding(n_acts + 1, emb_dim,
                                                 padding_idx=self.pad_idx)
            self.observation_embedding = nn.Embedding(n_lands + 1, emb_dim,
                                                      padding_idx=self.pad_idx)
        else:
...
class Decoder(nn.Module):
    ...
    def __init__(self, hidden_size=128, emb_dim=32, n_words=0, n_layers=1,
                 ...
                 max_length=0, cuda=False, sample_tokens=False):
        ...
        self.embedding = nn.Embedding(n_words, emb_dim, padding_idx=self.pad_idx)
    ...
    def forward(self, encoder_outputs=None, encoder_final=None,
        ...
        embedded = torch.Tensor(bsz, 1, self.emb_dim).fill_(self.vocab.tok2i[START_TOKEN])
...
class Seq2Seq(nn.Module):
    ...
    def __init__(self, n_lands=9, n_acts=3, n_words_trg=0, hidden_size=0,
                 ...
                 orientation_aware=True, sample_tokens=False,
                 **kwargs):
